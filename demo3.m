...
subplot(2,1,1)
legend(c1)
title('不同步长的LMS算法滤波器参数收敛情况')
subplot(2,1,2)
legend(c1)
title('不同步长的LMS算法误差收敛情况')
...
subplot(2,1,1)
legend(c2)
title('不同步长的NLMS算法滤波器参数收敛情况')
subplot(2,1,2)
legend(c2)
title('不同步长的NLMS算法误差收敛情况')
...
subplot(2,1,1)
legend(c3)
title('不同权重的RLS算法滤波器参数收敛情况')
subplot(2,1,2)
legend(c3)
title('不同权重的RLS算法误差收敛情况')
...
plot(w3(M,:)','LineWidth', 3);
legend({'NLMS','LMS','RLS'})
title('滤波器参数收敛情况对比：\mu1 = 0.01, \mu2 = 0.1, \lambda = 0.1')
subplot(2,1,2)
plot(e2);
hold on
plot(e1);
plot(e3);
legend({'NLMS','LMS','RLS'})
title('误差收敛情况对比：\mu1 = 0.01, \mu2 = 0.1,\lambda = 0.1')
